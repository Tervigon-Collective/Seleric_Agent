...
| B2 | **Resolved in the backend (2026-09-25):** `conversations.py` now emits `answer.delta` / `answer.reset`. **Do not speak deltas** — they are pre-validation (`validation/__init__.py`), and audio cannot be unsaid after a REVISE. Voice speaks only the final validated message. Originally: nothing emitted it | `src/seleric_swarm/conversations/event_mapping_v1.py:43`; no other hit in `src/` | Phase 1 accepts it; Phase 2 fixes it |
...
> **Update 2026-09-25:** superseded. The loop is now instrumented (see the B7 row). The vocabulary in use is `agent.tool_started`, `agent.tool_completed`, `agent.answering`, plus `run.*` and `answer.*`. The text below is kept as history.
