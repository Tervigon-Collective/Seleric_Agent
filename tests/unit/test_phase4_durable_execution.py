...
def test_recovery_marks_expired_attempt_retryable_or_failed():
    ...
    assert next_attempt.status is RunAttemptStatus.RETRYABLE
...
def test_lease_version_fences_stale_heartbeats_and_completion():
    repositories, run, attempt = _run_records()
    now = datetime.now(UTC)
    claimed = repositories.runs.claim(run.id, "worker-a", 10, now=now)
    assert claimed is not None
    assert not repositories.runs.heartbeat(
        attempt.id,
        "worker-a",
        10,
...
@pytest.mark.asyncio
async def test_recovery_worker_claims_and_executes_resumable_attempt():
    repositories, run, _ = _run_records()
    now = datetime.now(UTC)
    repositories.runs.claim(run.id, "dead-worker", 1, now=now - timedelta(seconds=2))
    RunRecoveryService(repositories.runs, retry_delay_s=0).recover_expired(now=now)
    executed: list[tuple[str, int]] = []

    async def execute(resumed_run, resumed_attempt):
        executed.append((resumed_run.id, resumed_attempt.attempt_number))
    ...
    assert executed == [(run.id, 2)]
    assert repositories.runs.get(run.id).status is RunStatus.COMPLETED


@pytest.mark.asyncio
async def test_inprocess_queue_enqueues_through_fenced_worker():
    repositories, run, attempt = _run_records()
    repositories.runs.update_attempt(
        attempt.model_copy(update={"status": RunAttemptStatus.RETRYABLE})
    )
    executed = asyncio.Event()

    async def execute(_run, _attempt):
        executed.set()
...
@pytest.mark.asyncio
async def test_worker_lease_loss_fences_terminal_commit():
    repositories, run, attempt = _run_records()
    repositories.runs.update_attempt(
        attempt.model_copy(update={"status": RunAttemptStatus.RETRYABLE})
    )
    terminal_calls: list[RunStatus] = []

    async def lose_lease(_run, claimed):
        repositories.runs.update_attempt(
            claimed.model_copy(
                update={"worker_id": "replacement", "version": claimed.version + 1}
            )
        )
        return RunExecutionResult(on_committed=terminal_calls.append)
    ...
    assert repositories.runs.get(run.id).status is not RunStatus.COMPLETED


@pytest.mark.asyncio
async def test_worker_crash_creates_resumable_attempt_then_completes():
    repositories, run, attempt = _run_records(max_attempts=2)
    repositories.runs.update_attempt(
        attempt.model_copy(update={"status": RunAttemptStatus.RETRYABLE})
    )

    async def crash(_run, _attempt):
        raise RuntimeError("worker crashed")

    first = RunRecoveryWorker(
        repositories.runs,
        crash,
        worker_id="crashing-worker",
        retry_delay_s=0,
    )
    failed = await first.run_once()
    assert failed.retryable == 1
    next_attempt = repositories.runs.list_recoverable()[0]
    assert next_attempt.attempt_number == 2

    resumed: list[int] = []

    async def resume(_run, claimed):
        resumed.append(claimed.attempt_number)
        return RunExecutionResult()

    second = RunRecoveryWorker(
        repositories.runs,
        resume,
        worker_id="replacement-worker",
        retry_delay_s=0,
    )
    completed = await second.run_once()
...
@pytest.mark.asyncio
async def test_worker_cancellation_boundary_commits_once():
    repositories, run, attempt = _run_records()
    repositories.runs.update_attempt(
        attempt.model_copy(update={"status": RunAttemptStatus.RETRYABLE})
    )
    ...
    async def cancel_at_boundary(_run, _attempt):
        return RunExecutionResult(
            status=RunStatus.CANCELLED,
            error_code="CANCELLED",
            on_committed=terminal_calls.append,
        )
    ...
    assert terminal_calls == [RunStatus.CANCELLED]
...
def test_production_agents_compile_with_injected_checkpointer(monkeypatch):
    from seleric_swarm.agents.diagnostic import agent as diagnostic_module
    from seleric_swarm.agents.prediction import agent as prediction_module
    from seleric_swarm.agents.skeptic import agent as skeptic_module
    ...
    monkeypatch.setattr(
        ...
        lambda *, checkpointer=None: compiled if checkpointer is marker else None,
    )
    monkeypatch.setattr(
        prediction_module,
    ...
    assert diagnostic_module.DiagnosticAgent(checkpointer=marker)._graph() is compiled
    assert prediction_module.PredictionAgent(checkpointer=marker)._graph() is compiled
    assert skeptic_module.SkepticAgent(checkpointer=marker)._graph() is compiled
