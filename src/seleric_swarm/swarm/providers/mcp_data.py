...
In production/staging, only live Seleric catalogue values are returned.
Fixture mode keeps the offline scenario providers for tests/replay.
"""
...
@dataclass
class McpFetchStats:
    """Per-mission counters for MCP vs missing live data (no secrets)."""
...
class EmptyDataProvider:
    """Live mode: domains without a Seleric module return no readings (never fixtures)."""
...
class HybridMcpDataProvider:
    ...
    async def fetch(
        ...
        *,
        ...
        end = str(time_range.get("end") or time_range.get("start") or "")[:10]
...
def build_hybrid_bundle(
    ...
    *,
    ...
    """Build providers: live MCP for domains with a seleric_module; empty otherwise."""
    ...
    for d in domains:
        ...
        if mode == "fixture" or mcp is None:
            data[d] = FixtureDataProvider(d, scenario)
        elif cfg and cfg.seleric_module:
