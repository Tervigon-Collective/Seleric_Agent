"""Finance/profitability domain expertise and governed data access."""
...
AGENT_VERSION = "0.1.0"
ALLOWED_METRICS = {"metric.net_profit", "metric.contribution_margin", "metric.cogs", "metric.payment_fees"}
    ...
    async def run(self, ctx: AgentContext) -> dict[str, Any]:
        ...
        if metric_id and metric_id not in ALLOWED_METRICS:
            return {
                ...
                "error_message": f"Finance agent does not own {metric_id}",
                "unsupported_reason": f"Metric {metric_id} is outside finance allowlist",
